...
# Ces classes gèrent le chargement correct
# de l'objet BurgerSolutionInterpolator, même si défini dans __main__
class BurgerSolutionInterpolator: # Forward declaration for unpickler
...
class _PatchedUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        try:
            ...
            # This might be risky if the module has side effects or heavy imports not needed for unpickling.
            # For BurgerSolutionInterpolator, it's meant to point to the class in *this* file.
            # If the original pickle was from a script where BurgerSolutionInterpolator was in __main__,
            # this unpickler redirects it. If it was pickled with a proper module path,
            # Python's default unpickler should find it if that module is importable.
            # The __import__(module) line is a fallback.
            try:
                __import__(module)
                return super().find_class(module, name)
            except ImportError: # If module truly cannot be imported
...
class BurgerSolutionInterpolator: # Actual definition
    ...
    def _torch_interp1d(self, x_query: torch.Tensor, xp: torch.Tensor, fp_matrix: torch.Tensor) -> torch.Tensor:
        ...
        """
        # N_query = x_query.shape[0] # Unused variable
        M_xp = xp.shape[0]
        ...
        if M_xp == 1: # All points interpolate to the single value in fp_matrix
            return fp_matrix[:, 0]
        ...
        results[right_extrap_mask] = fp_matrix[right_extrap_mask, M_xp - 1]
        ...
        if torch.any(interp_mask):
            ...
            left_indices = torch.clamp(right_indices - 1, 0, M_xp - 2)
    ...
    def evaluate(self, points: torch.Tensor) -> torch.Tensor:
        ...
        # N_points = points.shape[0] # Unused variable
        times_req = points[:, 0]
        coords_req = points[:, 1]
        ...
        before_start = times_req < self.times[0] 
        after_end = times_req > self.times[-1]  

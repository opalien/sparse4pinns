...
# Import from the new FEniCS-free interpolator file
from solution_interpolator import BurgerSolutionInterpolator, load_solution_interpolator

# Désactiver la journalisation FEniCS trop verbeuse si souhaité
fn.set_log_level(fn.LogLevel.WARNING) # Ou ERROR

# SOLVER_DIR is used by the __main__ block to define output_pickle_file path
SOLVER_DIR = os.path.join('examples', 'any', 'solvers')

# --- Pickle loading classes and BurgerSolutionInterpolator class are now in solution_interpolator.py ---
# --- So, lines 17-258 of the original file are removed here. ---
...
if __name__ == '__main__':
    ...
    # Ensure pickle_path uses the SOLVER_DIR defined in this file
    ...
    if saved_path and os.path.exists(saved_path):
        ...
        try:
            ...
            # Ensure test_points are created with the interpolator's dtype for consistency if possible,
            # or that the interpolator handles dtype conversion robustly.
            # The interpolator's evaluate method converts to its own dtype and device.
            test_points = torch.tensor(test_points_data, dtype=torch.float64) 
            ...
            if loaded_interpolator:
                 ...
                 plt.xlabel('x')
                 plt.ylabel('u(t, x)')
                 plt.title(f'Burger Solution Profiles (P2, CN, nx={loaded_interpolator.num_spatial_points-1}, dt={(loaded_interpolator.t_max)/(loaded_interpolator.num_time_steps-1 if loaded_interpolator.num_time_steps > 1 else 1):.2e})') # Avoid div by zero for num_time_steps=1
                 plt.legend()
                 plt.grid(True)
                 plt.ylim(-1.1, 1.1)
                 plt.show()
